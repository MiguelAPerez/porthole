const fs = require('fs');
const path = require('path');
const os = require('os');
const { execSync } = require('child_process');
const { detectTechStack, getTechIcon, getTechColor } = require('./detector');

function extractDescription(pkg, composer, dirPath, readme) {
  // 1. package.json (Node/JS)
  if (pkg && pkg.description) return pkg.description;
  
  // 2. composer.json (PHP)
  if (composer && composer.description) return composer.description;
  
  // 3. Cargo.toml (Rust)
  const cargoPath = path.join(dirPath, 'Cargo.toml');
  if (fs.existsSync(cargoPath)) {
    try {
      const content = fs.readFileSync(cargoPath, 'utf8');
      const match = content.match(/^description\s*=\s*"(.*?)"/m);
      if (match) return match[1];
    } catch { /* ignore */ }
  }

  // 4. pyproject.toml (Python)
  const pyPath = path.join(dirPath, 'pyproject.toml');
  if (fs.existsSync(pyPath)) {
    try {
      const content = fs.readFileSync(pyPath, 'utf8');
      const match = content.match(/^description\s*=\s*"(.*?)"/m);
      if (match) return match[1];
    } catch { /* ignore */ }
  }

  // 5. README.md fallback
  if (readme) {
    // Strip # Header and find first real text line
    const lines = readme.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'));
    if (lines.length > 0) {
      const firstLine = lines[0];
      if (firstLine.length > 5) return firstLine; // Avoid tiny snippets
    }
  }

  return '';
}

function getGitRemoteUrl(dirPath) {
    try {
        return execSync('git remote get-url origin', { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
    } catch {
        try {
            const out = execSync('git remote -v', { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }).toString();
            const line = out.split('\n').find(l => l.includes('(fetch)'));
            if (!line) return null;
            return line.split(/\s+/)[1];
        } catch {
            return null;
        }
    }
}

function classifyGitHost(host) {
    const h = host.toLowerCase();
    if (h === 'github.com') return 'GitHub';
    if (h.includes('gitea')) return 'Gitea';
    if (h.includes('gitlab')) return 'GitLab';
    if (h.includes('bitbucket')) return 'Bitbucket';
    return 'Git server';
}

function applyGitHostMap(host, hostMap = {}) {
    const key = host.toLowerCase();
    for (const [from, to] of Object.entries(hostMap)) {
        if (from.toLowerCase() === key) return to;
    }
    return host;
}

function parseGitRemoteUrl(remote, hostMap = {}) {
    if (!remote) return null;
    const raw = remote.trim();

    let host;
    let repoPath;

    const httpsMatch = raw.match(/^https?:\/\/([^/]+)\/(.+?)(?:\.git)?\/?$/i);
    if (httpsMatch) {
        [, host, repoPath] = httpsMatch;
    } else {
        const scpMatch = raw.match(/^[^@]+@([^:]+):(.+?)(?:\.git)?$/);
        if (scpMatch) {
            [, host, repoPath] = scpMatch;
        } else {
            const bareScpMatch = raw.match(/^([^@:/\s]+):(.+?)(?:\.git)?$/);
            if (bareScpMatch) {
                [, host, repoPath] = bareScpMatch;
            } else {
            const sshMatch = raw.match(/^ssh:\/\/[^@]+@([^/]+)\/(.+?)(?:\.git)?\/?$/i);
            if (sshMatch) {
                [, host, repoPath] = sshMatch;
                host = host.split(':')[0];
            }
            }
        }
    }

    if (!host || !repoPath) return null;

    repoPath = repoPath.replace(/\.git$/, '');
    const webHost = applyGitHostMap(host, hostMap);
    const webUrl = `https://${webHost}/${repoPath}`;
    return { webUrl, host: classifyGitHost(webHost) };
}

function getGitInfo(dirPath, hostMap = {}) {
    const gitDir = path.join(dirPath, '.git');
    if (!fs.existsSync(gitDir)) return null;

    try {
        const branch = execSync('git rev-parse --abbrev-ref HEAD', { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
        const status = execSync('git status --short', { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
        const lastCommit = execSync('git log -1 --format=%s', { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
        const remote = parseGitRemoteUrl(getGitRemoteUrl(dirPath), hostMap);

        return {
            branch,
            isDirty: status.length > 0,
            lastCommit: lastCommit.slice(0, 50) + (lastCommit.length > 50 ? '...' : ''),
            ...(remote || {})
        };
    } catch (e) {
        return null;
    }
}

function getLastModified(dirPath) {
  let newest = 0;
  try {
    const s = fs.statSync(dirPath);
    newest = s.mtimeMs;
    // Check top level files only for speed
    const entries = fs.readdirSync(dirPath, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.name === 'node_modules' || entry.name === '.git') continue;
      try {
        const stat = fs.statSync(path.join(dirPath, entry.name));
        if (stat.mtimeMs > newest) newest = stat.mtimeMs;
      } catch { /* ignore */ }
    }
  } catch { /* ignore */ }
  return newest;
}

function formatLocalDate(msOrDate) {
  const d = msOrDate instanceof Date ? msOrDate : new Date(msOrDate);
  const y = d.getFullYear();
  const m = String(d.getMonth() + 1).padStart(2, '0');
  const day = String(d.getDate()).padStart(2, '0');
  return `${y}-${m}-${day}`;
}

function emptyDay() {
  return { claude: 0, antigravity: 0, cursor: 0 };
}

function bumpDay(dailyActivity, date, field) {
  if (!dailyActivity[date]) dailyActivity[date] = emptyDay();
  dailyActivity[date][field] += 1;
}

function getAntigravityUsage(devDir) {
  const usageMap = {};
  const dailyActivity = {};
  const brainDir = path.join(os.homedir(), '.gemini', 'antigravity', 'brain');
  const conversationsDir = path.join(os.homedir(), '.gemini', 'antigravity', 'conversations');

  if (!fs.existsSync(brainDir)) return { usageMap, dailyActivity };

  try {
    const entries = fs.readdirSync(brainDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;

      const brainPath = path.join(brainDir, entry.name);

      let sessionTokens = 0;
      const pbPath = path.join(conversationsDir, `${entry.name}.pb`);
      if (fs.existsSync(pbPath)) {
        try {
          const stats = fs.statSync(pbPath);
          sessionTokens = Math.floor(stats.size / 4);
        } catch (e) { /* ignore */ }
      }

      // Use brain folder mtime as session date
      try {
        const stat = fs.statSync(brainPath);
        const date = formatLocalDate(stat.mtimeMs);
        bumpDay(dailyActivity, date, 'antigravity');
      } catch (e) { /* ignore */ }

      const filesToTry = ['task.md.resolved.0', 'task.md', 'implementation_plan.md.resolved', 'implementation_plan.md'];
      const foundProjects = new Set();

      for (const fileName of filesToTry) {
        const filePath = path.join(brainPath, fileName);
        if (fs.existsSync(filePath)) {
          try {
            const content = fs.readFileSync(filePath, 'utf8');
            const escapedDir = devDir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const re = new RegExp(`file://${escapedDir}/([^/\\s)\\]'"]+)`, 'g');
            const matches = content.matchAll(re);
            for (const match of matches) {
              foundProjects.add(match[1]);
            }
          } catch (e) { /* ignore */ }
        }
      }

      for (const projectName of foundProjects) {
        if (!usageMap[projectName]) {
          usageMap[projectName] = { sessions: 0, tokens: 0 };
        }
        usageMap[projectName].sessions += 1;
        usageMap[projectName].tokens += sessionTokens;
      }
    }
  } catch (e) {
    console.error('Error scanning Antigravity brain:', e.message);
  }
  return { usageMap, dailyActivity };
}

function getClaudeCLIUsage(projectNames, devDir) {
  const projectMap = {};
  const dailyActivity = {};
  const claudeDir = path.join(os.homedir(), '.claude', 'projects');

  for (const name of projectNames) {
    projectMap[name] = { sessions: 0, inputTokens: 0, outputTokens: 0, cacheHits: 0 };
  }

  if (!fs.existsSync(claudeDir)) return { projectMap, dailyActivity };

  const sortedNames = [...projectNames].sort((a, b) => b.length - a.length);

  try {
    const projectDirs = fs.readdirSync(claudeDir, { withFileTypes: true });
    for (const entry of projectDirs) {
      if (!entry.isDirectory()) continue;

      const prefix = devDir.replace(/\//g, '-') + '-';
      if (!entry.name.startsWith(prefix)) continue;

      const fullFolderSuffix = entry.name.substring(prefix.length);

      let matchedProject = null;
      for (const name of sortedNames) {
        if (fullFolderSuffix === name || fullFolderSuffix.startsWith(name + '-')) {
          matchedProject = name;
          break;
        }
      }

      if (!matchedProject) continue;

      const projectPath = path.join(claudeDir, entry.name);

      try {
        const sessionFiles = fs.readdirSync(projectPath);
        for (const file of sessionFiles) {
          if (file.endsWith('.jsonl')) {
            projectMap[matchedProject].sessions++;
            try {
              const content = fs.readFileSync(path.join(projectPath, file), 'utf8');
              const lines = content.split('\n');
              let sessionDate = null;
              for (const line of lines) {
                if (!line.trim()) continue;
                try {
                  const obj = JSON.parse(line);
                  if (!sessionDate && obj.timestamp) {
                    sessionDate = formatLocalDate(obj.timestamp);
                    bumpDay(dailyActivity, sessionDate, 'claude');
                  }
                  if (obj.type === 'assistant' && obj.message?.usage) {
                    const usage = obj.message.usage;
                    projectMap[matchedProject].inputTokens += usage.input_tokens || 0;
                    projectMap[matchedProject].outputTokens += usage.output_tokens || 0;
                    projectMap[matchedProject].cacheHits += usage.cache_read_input_tokens || 0;
                  }
                } catch (e) { /* ignore malformed line */ }
              }
              if (!sessionDate) {
                try {
                  const stat = fs.statSync(path.join(projectPath, file));
                  bumpDay(dailyActivity, formatLocalDate(stat.mtimeMs), 'claude');
                } catch (e) { /* ignore */ }
              }
            } catch (e) { /* ignore file read error */ }
          }
        }
      } catch (e) { /* ignore project dir read error */ }
    }
  } catch (e) {
    console.error('Error scanning Claude CLI projects:', e.message);
  }
  return { projectMap, dailyActivity };
}

function encodeCursorProjectPath(devDir) {
  return devDir.replace(/^\//, '').replace(/[/.]/g, '-');
}

function getCursorUsage(projectNames, devDir) {
  const projectMap = {};
  const dailyActivity = {};
  const cursorDir = path.join(os.homedir(), '.cursor', 'projects');
  const cursorPrefix = encodeCursorProjectPath(devDir);

  for (const name of projectNames) {
    projectMap[name] = { sessions: 0 };
  }

  if (!fs.existsSync(cursorDir)) return { projectMap, dailyActivity };

  const sortedNames = [...projectNames].sort((a, b) => b.length - a.length);

  try {
    const entries = fs.readdirSync(cursorDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;

      const folderName = entry.name;
      if (folderName !== cursorPrefix && !folderName.startsWith(cursorPrefix + '-')) continue;

      let matchedProject = null;
      if (folderName.startsWith(cursorPrefix + '-')) {
        const suffix = folderName.substring(cursorPrefix.length + 1);
        for (const name of sortedNames) {
          if (suffix === name || suffix.startsWith(name + '-')) {
            matchedProject = name;
            break;
          }
        }
      }

      const transcriptsDir = path.join(cursorDir, folderName, 'agent-transcripts');
      if (!fs.existsSync(transcriptsDir)) continue;

      try {
        const sessions = fs.readdirSync(transcriptsDir, { withFileTypes: true });
        for (const session of sessions) {
          if (!session.isDirectory()) continue;

          const jsonlPath = path.join(transcriptsDir, session.name, `${session.name}.jsonl`);
          if (!fs.existsSync(jsonlPath)) continue;

          try {
            const stat = fs.statSync(jsonlPath);
            bumpDay(dailyActivity, formatLocalDate(stat.mtimeMs), 'cursor');
            if (matchedProject) projectMap[matchedProject].sessions += 1;
          } catch (e) { /* ignore */ }
        }
      } catch (e) { /* ignore transcripts dir read error */ }
    }
  } catch (e) {
    console.error('Error scanning Cursor projects:', e.message);
  }

  return { projectMap, dailyActivity };
}

function getGitCommitsByDay(devDir) {
  const commitsByDay = {};
  let entries;
  try {
    entries = fs.readdirSync(devDir, { withFileTypes: true });
  } catch { return commitsByDay; }

  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name.startsWith('.')) continue;
    const dirPath = path.join(devDir, entry.name);
    if (!fs.existsSync(path.join(dirPath, '.git'))) continue;
    try {
      const out = execSync(
        'git log --format="%ad" --date=short --after="26 weeks ago"',
        { cwd: dirPath, stdio: ['ignore', 'pipe', 'ignore'] }
      ).toString().trim();
      if (!out) continue;
      for (const date of out.split('\n')) {
        const d = date.trim();
        if (!d) continue;
        commitsByDay[d] = (commitsByDay[d] || 0) + 1;
      }
    } catch { /* ignore repos with no commits or git errors */ }
  }
  return commitsByDay;
}

function scanProjects(devDir, options = {}) {
  const gitHostMap = options.gitHostMap || {};
  const projects = [];
  const entries = fs.readdirSync(devDir, { withFileTypes: true });
  const projectNames = entries
    .filter(e => e.isDirectory() && !e.name.startsWith('.') && e.name !== 'node_modules' && e.name !== '.git')
    .map(e => e.name);

  const { usageMap: antigravityUsageMap, dailyActivity: agDailyActivity } = getAntigravityUsage(devDir);
  const { projectMap: claudeCLIUsageMap, dailyActivity: claudeDailyActivity } = getClaudeCLIUsage(projectNames, devDir);
  const { projectMap: cursorUsageMap, dailyActivity: cursorDailyActivity } = getCursorUsage(projectNames, devDir);
  const commitsByDay = getGitCommitsByDay(devDir);

  // Merge daily activity from all sources
  const dailyActivity = { ...claudeDailyActivity };
  for (const [date, counts] of Object.entries(agDailyActivity)) {
    if (!dailyActivity[date]) dailyActivity[date] = emptyDay();
    dailyActivity[date].antigravity += counts.antigravity;
  }
  for (const [date, counts] of Object.entries(cursorDailyActivity)) {
    if (!dailyActivity[date]) dailyActivity[date] = emptyDay();
    dailyActivity[date].cursor += counts.cursor;
  }
  for (const [date, count] of Object.entries(commitsByDay)) {
    if (!dailyActivity[date]) dailyActivity[date] = emptyDay();
    dailyActivity[date].commits = (dailyActivity[date].commits || 0) + count;
  }

  try {
    for (const entry of entries) {
      const dirPath = path.join(devDir, entry.name);

      // Skip if not a directory
      if (!entry.isDirectory()) continue;

      // Skip hidden directories and special folders
      if (entry.name.startsWith('.') ||
          entry.name === 'node_modules' ||
          entry.name === '.git') continue;

      const pkgPath = path.join(dirPath, 'package.json');
      const composerPath = path.join(dirPath, 'composer.json');

      let pkg = null;
      let composer = null;

      try {
        if (fs.existsSync(pkgPath)) {
          pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
        }
      } catch (e) { /* ignore */ }

      try {
        if (fs.existsSync(composerPath)) {
          composer = JSON.parse(fs.readFileSync(composerPath, 'utf8'));
        }
      } catch (e) { /* ignore */ }

      const readmePath = path.join(dirPath, 'README.md');
      let readme = '';
      try {
        if (fs.existsSync(readmePath)) {
          readme = fs.readFileSync(readmePath, 'utf8').slice(0, 2000);
        }
      } catch { /* ignore */ }

      const tech = detectTechStack(pkg, composer, dirPath);
      const description = extractDescription(pkg, composer, dirPath, readme);
      const git = getGitInfo(dirPath, gitHostMap);
      
      const agUsage = antigravityUsageMap[entry.name] || { sessions: 0, tokens: 0 };
      const cUsage = claudeCLIUsageMap[entry.name] || { sessions: 0, inputTokens: 0, outputTokens: 0, cacheHits: 0 };
      const curUsage = cursorUsageMap[entry.name] || { sessions: 0 };


      projects.push({
        name: entry.name,
        tech,
        icon: getTechIcon(tech),
        color: getTechColor(tech),
        description,
        readme,
        path: `file://${path.join(devDir, entry.name)}`,
        lastModified: getLastModified(dirPath),
        git,
        antigravityUsage: agUsage.sessions,
        antigravityTokens: agUsage.tokens,
        claudeSessions: cUsage.sessions,
        claudeInputTokens: cUsage.inputTokens,
        claudeOutputTokens: cUsage.outputTokens,
        claudeCacheHits: cUsage.cacheHits,
        cursorSessions: curUsage.sessions,
        indicators: {
            isDocker: fs.existsSync(path.join(dirPath, 'Dockerfile')) || fs.existsSync(path.join(dirPath, 'docker-compose.yml')),
            isTS: fs.existsSync(path.join(dirPath, 'tsconfig.json')),
            hasReadme: readme.length > 0
        }
      });
    }
  } catch (e) {
    console.error('Error scanning projects:', e.message);
  }

  projects.sort((a, b) => b.lastModified - a.lastModified);

  return { projects, dailyActivity };
}

module.exports = { scanProjects, extractDescription, parseGitRemoteUrl, classifyGitHost, applyGitHostMap };
