  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (!dailyActivity[date]) dailyActivity[date] = { claude: 0, antigravity: 0 };
        dailyActivity[date].antigravity += 1;
      } catch (e) { /* ignore */ }
              ...
              for (const line of lines) {
                ...
                try {
                  ...
                  if (!sessionDate && obj.timestamp) {
                    ...
                    if (!dailyActivity[sessionDate]) dailyActivity[sessionDate] = { claude: 0, antigravity: 0 };
                    dailyActivity[sessionDate].claude += 1;
                  }
...
function getGitCommitsByDay(devDir) {
  ...
  for (const entry of entries) {
    ...
    try {
      const out = execSync(
        'git log --format="%ad" --date=short --after="6 months ago"',
