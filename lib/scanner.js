...
function getAntigravityUsage() {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Heuristic: Estimate tokens from the .pb conversation file size (ID matches brain folder name)
      let sessionTokens = 0;
      const pbPath = path.join(conversationsDir, `${entry.name}.pb`);
      if (fs.existsSync(pbPath)) {
        try {
          const stats = fs.statSync(pbPath);
          sessionTokens = Math.floor(stats.size / 4); // 1 token per 4 bytes estimate
