...
const TEXT_EXTS = new Set(['.js', '.ts', '.jsx', '.tsx', '.py', '.go', '.rs', '.rb', '.php',
  ...
  '.toml', '.sh', '.bash', '.zsh', '.scss', '.vue', '.svelte', '.sql',
...
async function bulkIngestFiles(files, locusUrl, spaceName, apiKey) {
  const form = new FormData();
  for (const { relPath, content } of files) {
    form.append('files', new Blob([content], { type: 'text/plain' }), relPath);
  }
  const res = await fetch(`${locusUrl}/spaces/${spaceName}/documents/bulk`, {
    method: 'POST',
    headers: authHeaders(apiKey),
    body: form
  });
  ...
  return results.map(r => ({ relPath: r.filename, docId: r.doc_id, error: r.error }));
}
...
const DEFAULT_MAX_FILE_KB = 50;

function walkProject(projectDir, maxFileKb = DEFAULT_MAX_FILE_KB) {
  const MAX_FILE_BYTES = maxFileKb * 1024;
...
async function syncToLocus(projects, locusUrl, locusSpace, cachePath, apiKey = '', maxFileKb = DEFAULT_MAX_FILE_KB) {
  ...
  for (const p of projects) {
    ...
    const { files: currentFiles, skipped } = walkProject(projectDir, maxFileKb);
    ...
    for (const relPath of Object.keys(fileCache)) {
      if (!currentFiles[relPath] || currentFiles[relPath].hash !== fileCache[relPath].hash) {
        if (fileCache[relPath].docId && fileCache[relPath].docId !== 'FAILED') {
    ...
    if (total) {
      ...
      for (let i = 0; i < toIngest.length; i += BATCH_SIZE) {
        ...
        const batchFiles = [];
        for (const [relPath, { fullPath }] of batch) {
          let content;
          try { content = fs.readFileSync(fullPath, 'utf8'); } catch { done++; printBar(); continue; }
          batchFiles.push({ relPath, content });
        }
        try {
          const results = await bulkIngestFiles(batchFiles, locusUrl, spaceName, apiKey);
          for (const { relPath, docId, error } of results) {
            const entry = toIngest.find(([r]) => r === relPath);
            if (error) {
              errors.push(`  ✗ ${relPath}: ${error}`);
              fileCache[relPath] = { hash: entry ? entry[1].hash : '', docId: 'FAILED' };
            } else {
              fileCache[relPath] = { hash: entry ? entry[1].hash : '', docId };
              uploaded++;
            }
          }
        } catch (e) {
          for (const { relPath } of batchFiles) errors.push(`  ✗ ${relPath}: ${e.message}`);
        }
        done += batch.length;
        printBar();
      }
      const serverErrors = errors.filter(e => e.includes('500'));
      const otherErrors = errors.filter(e => !e.includes('500'));
      ...
      if (serverErrors.length) console.log(`  ⚠ ${serverErrors.length} file(s) skipped (embedding server error)`);
      if (otherErrors.length) console.log(otherErrors.join('\n'));
